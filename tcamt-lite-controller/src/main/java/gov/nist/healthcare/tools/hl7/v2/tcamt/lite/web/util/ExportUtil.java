...
public class ExportUtil {
  ...
  public nu.xom.Document serializeProfileToDoc(Profile profile) {
    ...
    for (Datatype d : profile.getDatatypes().getChildren()) {
      ds.appendChild(
          this.serializeDatatypeForValidation(d, profile.getTables(), profile.getDatatypes()));
    }
    e.appendChild(ds);
  ...
  private void serializeProfileMetaData(nu.xom.Element e, ProfileMetaData metaData, String id) {
    ...
    elmMetaData
        .addAttribute(new Attribute("OrgName", !ExportUtil.str(metaData.getOrgName()).equals("")
            ? ExportUtil.str(metaData.getOrgName()) : "No Org Info"));
    elmMetaData
        .addAttribute(new Attribute("Version", !ExportUtil.str(metaData.getVersion()).equals("")
    ...
    if (metaData.getSpecificationName() != null && !metaData.getSpecificationName().equals(""))
      elmMetaData.addAttribute(
          new Attribute("SpecificationName", ExportUtil.str(metaData.getSpecificationName())));
  ...
  private nu.xom.Element serializeSegment(Segment s, Tables tables, Datatypes datatypes) {
      ...
      for (Mapping m : s.getDynamicMapping().getMappings()) {
        ...
        for (Case c : m.getCases()) {
          ...
          elmCase.addAttribute(
              new Attribute("Datatype", datatypes.findOne(c.getDatatype()).getLabel()));
    ...
    for (int i = 1; i < fields.size() + 1; i++) {
      ...
      elmField.addAttribute(new Attribute("Datatype",
          ExportUtil.str(datatypes.findOne(f.getDatatype().getId()).getLabel())));
  ...
  private nu.xom.Element serializeDatatypeForValidation(Datatype d, Tables tables,
      Datatypes datatypes) {
    ...
    if (d.getComponents() != null) {
      ...
      for (int i = 1; i < components.size() + 1; i++) {
        ...
        if (c.getTables() != null && c.getTables().size() > 0) {
          ...
          if (bindingStrength != null && !bindingStrength.equals(""))
            elmComponent
                .addAttribute(new Attribute("BindingStrength", ExportUtil.str(bindingStrength)));
          if (bindingLocation != null && !bindingLocation.equals(""))
            elmComponent
                .addAttribute(new Attribute("BindingLocation", ExportUtil.str(bindingLocation)));
        }
        if (c.isHide())
  ...
  public nu.xom.Element serializeTableLibraryToElement(Profile profile) {
    ...
    if (metadata == null) {
      ...
      if (profile.getMetaData().getStatus() != null
        ...
        elmMetaData.addAttribute(
            new Attribute("Status", ExportUtil.str(profile.getMetaData().getStatus())));
      if (profile.getMetaData().getTopics() != null
          && !profile.getMetaData().getTopics().equals(""))
        elmMetaData.addAttribute(
            new Attribute("Topics", ExportUtil.str(profile.getMetaData().getTopics())));
    }
    ...
    for (Table t : tableLibrary.getChildren()) {
      ...
      if (t != null) {
        ...
        elmValueSetDefinition.addAttribute(
            new Attribute("BindingIdentifier", ExportUtil.str(t.getBindingIdentifier())));
        ...
        if (t.getExtensibility() != null && !t.getExtensibility().equals(""))
          elmValueSetDefinition.addAttribute(
              new Attribute("Extensibility", ExportUtil.str(t.getExtensibility().value())));
        ...
        if (t.getCodes() != null) {
          for (Code c : t.getCodes()) {
            ...
            elmValueElement
                .addAttribute(new Attribute("DisplayName", ExportUtil.str(c.getLabel() + "")));
            ...
            if (c.getCodeUsage() != null && !c.getCodeUsage().equals(""))
              elmValueElement
                  .addAttribute(new Attribute("Usage", ExportUtil.str(c.getCodeUsage())));
  ...
  private void serializeCoConstaint(nu.xom.Element e, Profile profile) {
    ...
    for (Segment s : profile.getSegments().getChildren()) {
      if (s.getCoConstraints() != null) {
        if (s.getCoConstraints().getColumnList() != null
            && s.getCoConstraints().getColumnList().size() > 1) {
          ...
          for (CoConstraint cc : s.getCoConstraints().getConstraints()) {
            ...
            elmPlainCoConstraint
                .addAttribute(new Attribute("KeyValue", cc.getValues().get(0).getValue()));

            for (int i = 1; i < s.getCoConstraints().getColumnList().size(); i++) {
              String path =
                  s.getCoConstraints().getColumnList().get(i).getField().getPosition() + "[1]";
