...
angular.module('tcl').controller('TestPlanCtrl', function($document, $scope, $rootScope, $templateCache, Restangular, $http, $filter, $modal, $cookies, $timeout, userInfoService, ngTreetableParams, $interval, ViewSettings, StorageService, $q, notifications, IgDocumentService, ElementUtils, AutoSaveService, $sce) {
    ...
    $scope.getTemplateSourceName=function(){
     	angular.forEach($rootScope.igs, function(ig){

    		angular.forEach(ig.profile.messages.children, function(msg){
    			//console.log(msg);
    			if(msg.id===$rootScope.selectedTestStep.conformanceProfileId){
    				console.log("EQUAL");
    				$scope.tmpNAme=msg.name;
    				console.log($scope.tmpNAme);
    				return $scope.tmpNAme;
    			}
    ...
    $scope.saveAllChangedTestPlans = function() {
        ...
        $http.post('api/template/save', $rootScope.template).then(function(response) {}, function(error) {
            $rootScope.saved = false;
        });
                                ...
                                if (mapping.reference) {
                                    ...
                                    if (secondReferenceValue == null) {
                                        var caseFound = _.find(mapping.cases, function(c) {
                                            return referenceValue.split("^")[0] == c.value;
                                        });
                                        if (caseFound) {
    ...
    $scope.MessageOptions = [
        ...
        null, ['Overwrite', function($itemScope) {
    ...
    $scope.SegmentOptions = [
        ...
        null, ['Overwrite Template', function($itemScope) {
...
angular.module('tcl')
    .controller('MessageViewCtrl', function($scope, $rootScope) {
        $scope.loading = false;
        $scope.msg = null;
        $scope.messageData = [];
